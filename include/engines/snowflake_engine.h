#ifndef SNOWFLAKE_ENGINE_H
#define SNOWFLAKE_ENGINE_H
...
struct SnowflakeODBCHandles {
  SQLHENV env;
  SQLHDBC dbc;
};

class SnowflakeEngine : public IWarehouseEngine {
  std::string connectionString_;

public:
  explicit SnowflakeEngine(std::string connectionString);
  ~SnowflakeEngine();

  SnowflakeEngine(const SnowflakeEngine &) = delete;
  SnowflakeEngine &operator=(const SnowflakeEngine &) = delete;
...
private:
  std::unique_ptr<SnowflakeODBCHandles> createConnection();
  void executeODBCQuery(SQLHDBC dbc, const std::string &query);
  std::vector<std::vector<std::string>>
  executeODBCSelect(SQLHDBC dbc, const std::string &query);
  std::string mapDataType(const std::string &dataType);
  void cleanupConnection(SQLHENV env, SQLHDBC dbc);
};

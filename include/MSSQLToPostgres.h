...
private:
  ...
  void performBulkUpsert(pqxx::connection &pgConn,
    ...
    try {
      ...
      std::vector<std::string> pkColumns = getPrimaryKeyColumnsFromPostgres(
          pgConn, lowerSchemaName, tableName);
      ...
      std::string upsertQuery = buildUpsertQuery(columnNames, pkColumns, 
                                                lowerSchemaName, tableName);
      ...
      for (size_t batchStart = 0; batchStart < results.size(); 
        ...
        for (size_t i = batchStart; i < batchEnd; ++i) {
          ...
          if (row.size() != columnNames.size()) continue;
          ...
          for (size_t j = 0; j < row.size(); ++j) {
            ...
            if (row[j] == "NULL" || row[j].empty()) {
              ...
              std::string cleanValue = cleanValueForPostgres(row[j], columnTypes[j]);
      ...
      Logger::debug("performBulkUpsert", 
                    ...
                    " rows with UPSERT for " + sourceSchemaName + "." + tableName);
  ...
  void performBulkInsert(pqxx::connection &pgConn,
    ...
    try {
      ...
      txn.commit();
      Logger::debug("performBulkInsert", 
                    "Processed " + std::to_string(totalProcessed) + 
                    " rows with INSERT for " + lowerSchemaName + "." + tableName);
  ...
  std::vector<std::string> getPrimaryKeyColumnsFromPostgres(
      pqxx::connection &pgConn, const std::string &schemaName, 
      const std::string &tableName) {
    std::vector<std::string> pkColumns;
  ...
  std::string buildUpsertQuery(const std::vector<std::string> &columnNames,
                              ...
                              const std::string &tableName) {
    std::string query = "INSERT INTO \"" + schemaName + "\".\"" + tableName + "\" (";
  ...
  std::string buildUpsertConflictClause(const std::vector<std::string> &columnNames,
    ...
    for (size_t i = 0; i < columnNames.size(); ++i) {
      if (i > 0) conflictClause += ", ";
      conflictClause += "\"" + columnNames[i] + "\" = EXCLUDED.\"" + columnNames[i] + "\"";
    }
  ...
  std::string cleanValueForPostgres(const std::string &value, 
    ...
    if (upperType.find("BOOLEAN") != std::string::npos ||
      ...
      if (cleanValue == "0" || cleanValue == "false" || cleanValue == "FALSE" || 
          cleanValue.empty()) {
        return "NULL";
      } else if (cleanValue == "1" || cleanValue == "true" || cleanValue == "TRUE") {
        cleanValue = "1";
      } else {
        return "NULL";
      }

...
private:
  struct TimeColumnCache {
    std::string column;
    std::chrono::steady_clock::time_point timestamp;
  };
  
  std::unordered_map<std::string, TimeColumnCache> timeColumnCache;
  ...
  void syncCatalogMariaDBToPostgres() {
    try {
      ...
      for (const auto &connStr : mariaConnStrings) {
        ...
        Logger::info("syncCatalogMariaDBToPostgres",
                     "Found " + std::to_string(discoveredTables.size()) + " tables");

        // Preparar lista de tablas para batch processing
        std::vector<std::pair<std::string, std::string>> tablesToProcess;
        for (const std::vector<std::string> &row : discoveredTables) {
          if (row.size() >= 2) {
            tablesToProcess.emplace_back(row[0], row[1]);
          }
        }

        // Detectar columnas de tiempo en batch
        auto timeColumns = detectTimeColumnsBatchMariaDB(mariaConn, tablesToProcess);

        // Procesar tablas en batches para reducir transacciones
        const size_t BATCH_SIZE = 50;
        for (size_t i = 0; i < discoveredTables.size(); i += BATCH_SIZE) {
          size_t batchEnd = std::min(i + BATCH_SIZE, discoveredTables.size());
          
          pqxx::work txn(pgConn);
          for (size_t j = i; j < batchEnd; ++j) {
            const auto &row = discoveredTables[j];
            if (row.size() < 2) continue;

            std::string schemaName = row[0];
            std::string tableName = row[1];
            std::string timeColumn = timeColumns[schemaName + "." + tableName];
      ...
      // Log estadísticas del cache
      logCacheStats();
    } catch (const std::exception &e) {
      Logger::error("syncCatalogMariaDBToPostgres",
                   "Error in syncCatalogMariaDBToPostgres: " + std::string(e.what()));
    }
  }
...
private:
  std::unordered_map<std::string, std::string> detectTimeColumnsBatchMariaDB(
      MYSQL *conn, const std::vector<std::pair<std::string, std::string>> &tables) {
    std::unordered_map<std::string, std::string> results;
    
    if (tables.empty()) return results;
    
    try {
      // Construir query batch para todas las tablas
      ...
      for (size_t i = 0; i < tables.size(); ++i) {
        if (i > 0) batchQuery += " OR ";
        batchQuery += "(table_schema = '" + escapeSQL(tables[i].first) + 
                     "' AND table_name = '" + escapeSQL(tables[i].second) + "')";
      }
      
      batchQuery += ") AND COLUMN_NAME IN ('updated_at', 'created_at', 'modified_at', "
                   "'timestamp', 'last_modified', 'updated_time', 'created_time') "
                   "ORDER BY table_schema, table_name, "
                   "FIELD(COLUMN_NAME, 'updated_at', 'modified_at', 'last_modified', "
                   "'updated_time', 'created_at', 'created_time', 'timestamp')";
      
      auto queryResults = executeQueryMariaDB(conn, batchQuery);
      ...
      Logger::debug("detectTimeColumnsBatchMariaDB", 
                   "Detected time columns for " + std::to_string(results.size()) + 
                   " out of " + std::to_string(tables.size()) + " tables");
      
    } catch (const std::exception &e) {
      Logger::error("detectTimeColumnsBatchMariaDB",
                   "Error in batch time column detection: " + std::string(e.what()));
    }
    
    return results;
  }

  std::string getCachedTimeColumn(const std::string &schema, const std::string &table) {
    std::lock_guard<std::mutex> lock(cacheMutex);
    
    std::string key = schema + "." + table;
    auto it = timeColumnCache.find(key);
    
    if (it != timeColumnCache.end()) {
  ...
  void cacheTimeColumn(const std::string &schema, const std::string &table, 
                      const std::string &column) {
    std::lock_guard<std::mutex> lock(cacheMutex);
    
    std::string key = schema + "." + table;
    timeColumnCache[key] = {column, std::chrono::steady_clock::now()};
  }
  ...
  void cleanExpiredCache() {
    ...
    auto now = std::chrono::steady_clock::now();
    auto it = timeColumnCache.begin();
    size_t removed = 0;
    
    while (it != timeColumnCache.end()) {
      auto age = std::chrono::duration_cast<std::chrono::minutes>(now - it->second.timestamp);
      if (age.count() >= CACHE_TTL_MINUTES) {
        it = timeColumnCache.erase(it);
        removed++;
      } else {
        ++it;
      }
    ...
    if (removed > 0) {
      Logger::debug("cleanExpiredCache", 
                   "Removed " + std::to_string(removed) + " expired cache entries. "
                   "Cache size: " + std::to_string(timeColumnCache.size()));
    }
  }

  void logCacheStats() {
    std::lock_guard<std::mutex> lock(cacheMutex);
    Logger::info("logCacheStats", 
                "Time column cache size: " + std::to_string(timeColumnCache.size()) + 
                " entries");
  }
  ...
  std::string detectTimeColumnMariaDB(MYSQL *conn, const std::string &schema,
    ...
    std::string cached = getCachedTimeColumn(schema, table);
    if (!cached.empty()) {
      return cached;
    }
    ...
    try {
      ...
      if (!results.empty() && !results[0][0].empty()) {
        std::string timeColumn = results[0][0];
        cacheTimeColumn(schema, table, timeColumn);
        return timeColumn;
      } else {
        cacheTimeColumn(schema, table, "");
      }
    } catch (const std::exception &e) {

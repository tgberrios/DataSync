...
private:
  ...
  connectMSSQL(const std::string &connStr) {
    ...
    if (!SQL_SUCCEEDED(ret)) {
      Logger::error("connectMSSQL", "Failed to allocate ODBC environment handle");
      return std::unique_ptr<ODBCHandles, void (*)(ODBCHandles *)>(nullptr, [](ODBCHandles *) {});
    }
    ...
    if (!SQL_SUCCEEDED(ret)) {
      SQLFreeHandle(SQL_HANDLE_ENV, env);
      Logger::error("connectMSSQL", "Failed to allocate ODBC connection handle");
      return std::unique_ptr<ODBCHandles, void (*)(ODBCHandles *)>(nullptr, [](ODBCHandles *) {});
    }

    ret = SQLDriverConnect(dbc, NULL, (SQLCHAR *)connStr.c_str(),
    ...
    if (!SQL_SUCCEEDED(ret)) {
      ...
      SQLFreeHandle(SQL_HANDLE_ENV, env);
      Logger::error("connectMSSQL", "Failed to connect to MSSQL: " + std::string((char *)msg));
      return std::unique_ptr<ODBCHandles, void (*)(ODBCHandles *)>(nullptr, [](ODBCHandles *) {});
    }

    auto handles = new ODBCHandles{env, dbc};
    return std::unique_ptr<ODBCHandles, void (*)(ODBCHandles *)>(handles, [](ODBCHandles *h) {
      if (h) {
        SQLDisconnect(h->dbc);

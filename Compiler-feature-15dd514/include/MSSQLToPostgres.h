  ...
  void syncIndexesAndConstraints(const std::string &schema_name,
    ...
    if (!mssqlConn) {
      Logger::error("syncIndexesAndConstraints",
                    "Failed to connect to MSSQL for " + schema_name + "." + table_name);
      return;
    }
    ...
    try {
      ...
      for (const auto &table : tables) {
        ...
        Logger::debug("setupTableTargetMSSQLToPostgres", 
                      "Processing table " + table.schema_name + "." + table.table_name + 
                      " with database: " + databaseName + 
        ...
        std::string query =
            ...
            "WHERE s.name = '" + table.schema_name + "' AND t.name = '" + table.table_name + "' "
  ...
  void transferDataMSSQLToPostgres() {
    try {
      ...
      for (auto &table : tables) {
        ...
        auto countRes = executeQueryMSSQL(mssqlConn->dbc, "SELECT COUNT(*) FROM [" +
                                                         schema_name + "].[" +
                                                         table_name + "];");
        ...
        if (sourceCount == targetCount) {
          ...
          if (!table.last_sync_column.empty()) {
            ...
            try {
              ...
              auto mssqlMaxRes = executeQueryMSSQL(mssqlConn->dbc, mssqlMaxQuery);
...
private:
  ...
  executeQueryMSSQL(SQLHDBC conn, const std::string &query) {
    ...
    if (ret != SQL_SUCCESS && ret != SQL_SUCCESS_WITH_INFO) {
      ...
      Logger::error("executeQueryMSSQL", "SQLExecDirect failed - SQLState: " + std::string((char*)sqlState) + 

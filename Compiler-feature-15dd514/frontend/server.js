...
app.get("/api/dashboard/stats", async (req, res) => {
  try {
    ...
    const currentProcessText = currentProcessingTable.rows.length > 0
      ? `${currentProcessingTable.rows[0].schema_name}.${currentProcessingTable.rows[0].table_name} (${currentProcessingTable.rows[0].last_offset || 0} records)`
      : "No active transfers";

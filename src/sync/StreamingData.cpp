...
void StreamingData::initialize() {
  ...
  try {
    std::string connStr = DatabaseConfig::getPostgresConnectionString();
    vaultBuilder = std::make_unique<DataVaultBuilder>(connStr);
    Logger::info(LogCategory::MONITORING,
                 "Data vault builder initialized successfully");
  } catch (const std::exception &e) {
    Logger::error(LogCategory::MONITORING, "initialize",
                  "Error initializing data vault builder: " +
                      std::string(e.what()));
  }
...
void StreamingData::vaultBuilderThread() {
  Logger::info(LogCategory::MONITORING, "Vault builder thread started");
  while (running) {
    try {
      if (!vaultBuilder) {
        std::this_thread::sleep_for(std::chrono::seconds(60));
      ...
      auto vaultsResult = txn.exec(
          "SELECT vault_name, schedule_cron, metadata, active, enabled "
          "FROM metadata.data_vault_catalog "
          "WHERE active = true AND enabled = true");

      std::vector<DataVaultModel> manualBuilds;
      std::vector<DataVaultModel> scheduledBuilds;
      ...
      for (const auto &row : vaultsResult) {
        std::string vaultName = row["vault_name"].as<std::string>();
        std::string scheduleCron = row["schedule_cron"].is_null()
                                       ? ""
                                       : row["schedule_cron"].as<std::string>();
        std::string metadataStr = row["metadata"].is_null()
        ...
        if (metadata.contains("build_now") &&
            metadata["build_now"].get<bool>()) {
          try {
            DataVaultModel vault = vaultBuilder->getVault(vaultName);
            manualBuilds.push_back(vault);
          } catch (const std::exception &e) {
            Logger::error(LogCategory::MONITORING, "vaultBuilderThread",
                          "Error getting vault " + vaultName + ": " +
                              std::string(e.what()));
          }
        } else if (!scheduleCron.empty()) {
          try {
            DataVaultModel vault = vaultBuilder->getVault(vaultName);
            scheduledBuilds.push_back(vault);
          } catch (const std::exception &e) {
            Logger::error(LogCategory::MONITORING, "vaultBuilderThread",
                          "Error getting vault " + vaultName + ": " +
                              std::string(e.what()));
      ...
      for (const auto &vault : manualBuilds) {
        Logger::info(LogCategory::MONITORING, "vaultBuilderThread",
                     "Executing manual build: " + vault.vault_name);
        try {
          vaultBuilder->buildVault(vault.vault_name);
          pqxx::connection conn2(connStr);
          pqxx::work txn2(conn2);
          json updatedMetadata = vault.metadata;
          updatedMetadata["build_now"] = false;
          updatedMetadata.erase("build_timestamp");
          pqxx::params params;
          params.append(updatedMetadata.dump());
          params.append(vault.vault_name);
          txn2.exec(
              pqxx::zview("UPDATE metadata.data_vault_catalog SET metadata = "
              "$1::jsonb WHERE vault_name = $2"),
              params);
          txn2.commit();
        } catch (const std::exception &e) {
          Logger::error(LogCategory::MONITORING, "vaultBuilderThread",
      ...
      for (const auto &vault : scheduledBuilds) {
        if (vault.schedule_cron.empty()) {
          continue;
        }

        if (vault.metadata.contains("build_now") &&
            vault.metadata["build_now"].get<bool>()) {
          continue;
        }

        std::istringstream cronStream(vault.schedule_cron);
        std::string minute, hour, day, month, dow;
        cronStream >> minute >> hour >> day >> month >> dow;
        ...
        if (shouldRun) {
          Logger::info(LogCategory::MONITORING, "vaultBuilderThread",
                       "Executing scheduled build: " + vault.vault_name);
          try {
            vaultBuilder->buildVault(vault.vault_name);
          } catch (const std::exception &e) {
            Logger::error(LogCategory::MONITORING, "vaultBuilderThread",
      ...
      Logger::error(LogCategory::MONITORING, "vaultBuilderThread",
                    "Error in vault build cycle: " + std::string(e.what()));
    }

  ...
  try {
    ...
    for (size_t i = 0; i < deletedPKs.size(); ++i) {
      ...
      for (size_t j = 0; j < pkColumns.size(); ++j) {
        ...
        std::string pkColName = pkColumns[j];
        std::transform(pkColName.begin(), pkColName.end(), pkColName.begin(), ::tolower);
        if (value == "NULL" || value.empty()) {
          deleteQuery += txn.quote_name(pkColName) + " IS NULL";
        } else {
          deleteQuery +=
              txn.quote_name(pkColName) + " = " + txn.quote(value);
        }
      }
      deleteQuery += ")";
    }
    ...
    for (size_t i = 0; i < deletedPKs.size() && i < 3; ++i) {
      ...
      for (size_t j = 0; j < pkColumns.size(); ++j) {
        ...
        std::transform(pkColName.begin(), pkColName.end(), pkColName.begin(), ::tolower);
        if (value == "NULL" || value.empty()) {
          checkQuery += txn.quote_name(pkColName) + " IS NULL";
        } else {
          checkQuery +=
              txn.quote_name(pkColName) + " = " + txn.quote(value);
        }

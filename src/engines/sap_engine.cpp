...
SAPODBCConnection::SAPODBCConnection(const std::string &connectionString) {
  SQLRETURN ret = SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &env_);
  if (!SQL_SUCCEEDED(ret)) {
    Logger::error(LogCategory::DATABASE, "SAPODBCConnection",
...
SAPODBCConnection::~SAPODBCConnection() {
...
SAPODBCConnection::SAPODBCConnection(SAPODBCConnection &&other) noexcept
...
SAPEngine::SAPEngine(std::string connectionString)
    : connectionString_(std::move(connectionString)) {}

std::unique_ptr<SAPODBCConnection> SAPEngine::createConnection() {
  const int MAX_RETRIES = 3;
  const int INITIAL_BACKOFF_MS = 100;

  for (int attempt = 1; attempt <= MAX_RETRIES; ++attempt) {
    auto conn = std::make_unique<SAPODBCConnection>(connectionString_);
    if (conn->isValid()) {
      if (attempt > 1) {
        Logger::info(LogCategory::DATABASE, "SAPEngine",
    ...
    if (attempt < MAX_RETRIES) {
      ...
      Logger::warning(LogCategory::DATABASE, "SAPEngine",
  ...
  Logger::error(LogCategory::DATABASE, "SAPEngine",
...
SAPEngine::executeQuery(SQLHDBC dbc, const std::string &query) {
  ...
  if (!SQL_SUCCEEDED(ret)) {
    Logger::error(LogCategory::DATABASE, "SAPEngine", "Query execution failed");
...
std::vector<CatalogTableInfo> SAPEngine::discoverTables() {
  std::vector<CatalogTableInfo> tables;
  auto conn = createConnection();
  if (!conn)
    return tables;

  // SAP HANA uses standard INFORMATION_SCHEMA
  std::string query =
...
std::vector<std::string> SAPEngine::detectPrimaryKey(const std::string &schema,
...
std::string SAPEngine::detectTimeColumn(const std::string &schema,
                                       const std::string &table) {
  if (schema.empty() || table.empty()) {
    Logger::error(LogCategory::DATABASE, "SAPEngine",
...
std::pair<int, int> SAPEngine::getColumnCounts(const std::string &schema,
                                               const std::string &table,

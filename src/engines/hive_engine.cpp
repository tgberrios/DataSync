...
HiveODBCConnection::HiveODBCConnection(const std::string &connectionString) {
  SQLRETURN ret = SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &env_);
  if (!SQL_SUCCEEDED(ret)) {
    Logger::error(LogCategory::DATABASE, "HiveODBCConnection",
...
HiveODBCConnection::~HiveODBCConnection() {
...
HiveODBCConnection::HiveODBCConnection(HiveODBCConnection &&other) noexcept
...
HiveEngine::HiveEngine(std::string connectionString)
    : connectionString_(std::move(connectionString)) {}

std::unique_ptr<HiveODBCConnection> HiveEngine::createConnection() {
  const int MAX_RETRIES = 3;
  const int INITIAL_BACKOFF_MS = 100;

  for (int attempt = 1; attempt <= MAX_RETRIES; ++attempt) {
    auto conn = std::make_unique<HiveODBCConnection>(connectionString_);
    if (conn->isValid()) {
      if (attempt > 1) {
        Logger::info(LogCategory::DATABASE, "HiveEngine",
    ...
    if (attempt < MAX_RETRIES) {
      ...
      Logger::warning(LogCategory::DATABASE, "HiveEngine",
  ...
  Logger::error(LogCategory::DATABASE, "HiveEngine",
...
HiveEngine::executeQuery(SQLHDBC dbc, const std::string &query) {
  ...
  if (!SQL_SUCCEEDED(ret)) {
    Logger::error(LogCategory::DATABASE, "HiveEngine", "Query execution failed");
...
std::vector<CatalogTableInfo> HiveEngine::discoverTables() {
  std::vector<CatalogTableInfo> tables;
  auto conn = createConnection();
  if (!conn)
    return tables;

  // Hive uses INFORMATION_SCHEMA or SHOW TABLES
  std::string query =
...
std::vector<std::string> HiveEngine::detectPrimaryKey(const std::string &schema,
  ...
  // Hive may not support primary keys in older versions, try INFORMATION_SCHEMA
  std::string query =
      "SELECT COLUMN_NAME "
...
std::string HiveEngine::detectTimeColumn(const std::string &schema,
                                        const std::string &table) {
  if (schema.empty() || table.empty()) {
    Logger::error(LogCategory::DATABASE, "HiveEngine",
  ...
  std::string query =
      ...
      "OR DATA_TYPE LIKE '%DATE%' OR DATA_TYPE LIKE '%TIME%') "
...
std::pair<int, int> HiveEngine::getColumnCounts(const std::string &schema,
                                                 const std::string &table,

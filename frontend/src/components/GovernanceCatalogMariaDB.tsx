  ...
  useEffect(() => {
    const fetchDatabases = async () => {
      if (filters.server_name) {
        try {
          const databasesData = await governanceCatalogApi.getMariaDBDatabases(filters.server_name);
          ...
          items.map((item) => (
            <div key={item.id}>
              <TableRow $expanded={openItemId === item.id} onClick={() => toggleItem(item.id)}>
                ...
                <TableCell>{formatBytes(item.total_size_mb)}</TableCell>
                <TableCell>
                ...
                <TableCell>{item.engine || 'N/A'}</TableCell>
              </TableRow>
              <DetailsPanel $isOpen={openItemId === item.id}>
                <DetailGrid>
                  ...
                  <DetailValue>{formatBytes(item.data_size_mb)}</DetailValue>
                  
                  <DetailLabel>Index Size:</DetailLabel>
                  <DetailValue>{formatBytes(item.index_size_mb)}</DetailValue>
                  
                  <DetailLabel>Data Free:</DetailLabel>
                  <DetailValue>{formatBytes(item.data_free_mb)}</DetailValue>
                  
                  <DetailLabel>Fragmentation:</DetailLabel>
                  <DetailValue>{formatPercentage(item.fragmentation_pct)}</DetailValue>
                  
                  <DetailLabel>InnoDB Version:</DetailLabel>
                  <DetailValue>{item.innodb_version || 'N/A'}</DetailValue>
                  
                  <DetailLabel>InnoDB Page Size:</DetailLabel>
                  <DetailValue>{item.innodb_page_size ? `${item.innodb_page_size} bytes` : 'N/A'}</DetailValue>
                  
                  <DetailLabel>Table Reads:</DetailLabel>
                  <DetailValue>{formatNumber(item.table_reads)}</DetailValue>
                  
                  <DetailLabel>Table Writes:</DetailLabel>
                  <DetailValue>{formatNumber(item.table_writes)}</DetailValue>
                  
                  <DetailLabel>Index Reads:</DetailLabel>
                  <DetailValue>{formatNumber(item.index_reads)}</DetailValue>

...
public:
  ...
  void validateSchemaConsistency() {
    try {
      ...
      Logger::info(LogCategory::DATABASE, "Starting schema consistency validation");

      pqxx::work txn(pgConn);
      auto results = txn.exec(
          "SELECT schema_name, table_name, db_engine, connection_string, status "
          "FROM metadata.catalog "
          "WHERE active = true AND status IN ('PERFECT_MATCH', 'LISTENING_CHANGES', 'FULL_LOAD') "
      ...
      Logger::info(LogCategory::DATABASE, "Found " + std::to_string(totalTables) + " tables to validate");
      ...
      for (const auto &row : results) {
        ...
        Logger::info(LogCategory::DATABASE, "Validating schema: " + schemaName + "." + tableName + " [" + dbEngine + "]");
        ...
        if (dbEngine == "MariaDB") {
          auto counts = getColumnCountsMariaDB(connectionString, schemaName, tableName);
          sourceColumnCount = counts.first;
          targetColumnCount = counts.second;
        } else if (dbEngine == "MSSQL") {
          auto counts = getColumnCountsMSSQL(connectionString, schemaName, tableName);
          sourceColumnCount = counts.first;
          targetColumnCount = counts.second;
        } else if (dbEngine == "PostgreSQL") {
          auto counts = getColumnCountsPostgres(connectionString, schemaName, tableName);
          sourceColumnCount = counts.first;
          targetColumnCount = counts.second;
        }
        ...
        if (needsReset) {
          Logger::warning(LogCategory::DATABASE, "SCHEMA MISMATCH: " + schemaName + "." + tableName + 
          ...
          resetTxn.exec("DROP TABLE IF EXISTS \"" + escapeSQL(schemaName) + "\".\"" + escapeSQL(tableName) + "\"");
          ...
          resetTxn.exec("UPDATE metadata.catalog SET "
                       ...
                       "AND db_engine = '" + escapeSQL(dbEngine) + "'");
          resetTxn.commit();
          resetTables++;
        } else {
          Logger::info(LogCategory::DATABASE, "SCHEMA VALID: " + schemaName + "." + tableName + 
                      " - Columns match: " + std::to_string(sourceColumnCount));
          validatedTables++;
        }
      }

      Logger::info(LogCategory::DATABASE, "Schema validation completed - Validated: " + 
                  std::to_string(validatedTables) + ", Reset: " + std::to_string(resetTables) + 
      ...
      for (const auto &connStr : mariaConnStrings) {
        ...
        for (const std::vector<std::string> &row : discoveredTables) {
...

          {
            ...
            int64_t tableSize = 0;
            try {
              std::string sizeQuery = "SELECT table_rows FROM information_schema.tables WHERE table_schema = '" + 
              ...
              Logger::warning(LogCategory::DATABASE, "Could not get table size for " + schemaName + "." + tableName + ": " + std::string(e.what()));
            }
      ...
      for (const auto &connStr : mssqlConnStrings) {
        ...
        for (const std::vector<std::string> &row : discoveredTables) {
...

          {
            ...
            int64_t tableSize = 0;
            try {
              std::string sizeQuery = "SELECT SUM(rows) FROM sys.dm_db_partition_stats WHERE object_id = OBJECT_ID('" + 
      ...
      for (const auto &connStr : pgConnStrings) {
        ...
        for (const auto &row : discoveredTables) {
...

          {
            ...
            int64_t tableSize = 0;
            try {
              std::string sizeQuery = "SELECT n_tup_ins + n_tup_upd + n_tup_del FROM pg_stat_user_tables WHERE schemaname = '" + 
                                    escapeSQL(schemaName) + "' AND relname = '" + escapeSQL(tableName) + "'";
              auto sizeResult = txn.exec(sizeQuery);
              if (!sizeResult.empty() && !sizeResult[0][0].is_null()) {
                tableSize = sizeResult[0][0].as<int64_t>();
              }
...
private:
  ...
  std::pair<int, int> getColumnCountsMariaDB(const std::string &connectionString, 
                                            const std::string &schema, 
                                            const std::string &table) {
    try {
      std::string host, user, password, db, port;
      ...
      if (!sourceResults.empty() && !sourceResults[0].empty()) {
        try { sourceCount = std::stoi(sourceResults[0][0]); } catch (...) { sourceCount = 0; }
      }

      pqxx::connection pgConn(DatabaseConfig::getPostgresConnectionString());
      pqxx::work txn(pgConn);
      std::string targetQuery = "SELECT COUNT(*) FROM information_schema.columns "
                               "WHERE table_schema = '" + escapeSQL(schema) + "' "
                               "AND table_name = '" + escapeSQL(table) + "'";
  ...
  std::pair<int, int> getColumnCountsMSSQL(const std::string &connectionString, 
    ...
    try {
      ...
      ret = SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE, &env);
      if (!SQL_SUCCEEDED(ret)) return {0, 0};
      ...
      std::string sourceQuery = "SELECT COUNT(*) FROM sys.columns c "
                               "INNER JOIN sys.tables t ON c.object_id = t.object_id "
                               "INNER JOIN sys.schemas s ON t.schema_id = s.schema_id "
  ...
  std::pair<int, int> getColumnCountsPostgres(const std::string &connectionString, 
                                             const std::string &schema, 
                                             const std::string &table) {
    try {
      pqxx::connection sourceConn(connectionString);
      if (!sourceConn.is_open()) return {0, 0};
